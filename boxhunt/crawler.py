...
class BoxHuntCrawler:
    ...
    def __init__(self, metadata_file: str = None, enabled_sources: list[str] = None):
        ...
        self.image_processor.downloaded_hashes = (
            self.storage_manager.get_existing_hashes()
        )
    ...
    async def crawl_single_keyword(
        self, keyword: str, max_images_per_source: int = 20, process_images: bool = True
    ) -> dict[str, Any]:
    ...
    async def crawl_multiple_keywords(
        ...
        delay_between_keywords: float = None,
    ) -> dict[str, Any]:
        ...
        for i, keyword in enumerate(keywords):
            try:
                result = await self.crawl_single_keyword(keyword, max_images_per_source)
    ...
    async def _process_with_progress(
        self, search_results: list[ImageResult], description: str
    ) -> list[dict[str, Any]]:
        ...
        # Create progress bar
        pbar = tqdm(total=len(search_results), desc=description, unit="img")
    ...
    async def resume_crawl(
        self, keywords: list[str] = None, max_images_per_source: int = 20
    ) -> dict[str, Any]:
    ...
    def export_results(self, format: str = "csv") -> str | None:
