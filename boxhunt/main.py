...
async def cmd_crawl(args):
    ...
    if keywords and len(keywords) == 1:
        ...
        print(f"  Keywords processed: {result['completed_keywords']}/{result['total_keywords']}")
...
def cmd_stats(args):
    ...
    print(f"  Average dimensions: {stats['storage']['avg_width']}×{stats['storage']['avg_height']}")
...
def cmd_config(args):
    ...
    print(f"  English: {', '.join(Config.KEYWORDS_EN[:5])}{'...' if len(Config.KEYWORDS_EN) > 5 else ''}")
    print(f"  Chinese: {', '.join(Config.KEYWORDS_CN[:3])}{'...' if len(Config.KEYWORDS_CN) > 3 else ''}")
...
def create_parser():
    ...
    crawl_parser.add_argument('--keywords', type=str,
                             help='Comma-separated keywords (default: use all predefined)')
    crawl_parser.add_argument('--max-images', type=int, default=20,
    ...
    resume_parser.add_argument('--keywords', type=str,
                              help='Comma-separated keywords (default: use all predefined)')
    ...
    export_parser.add_argument('--format', choices=['csv', 'json', 'xlsx'], 
